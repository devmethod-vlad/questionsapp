...
It contains:
- FastAPI settings (`Settings`) for ASGI runtime.
- Legacy Flask-compatible config classes (`Config`, `ProdConfig`, `DevConfig`)
  consumed by bridge code and Celery tasks during the migration period.
"""
...
class Config:
    """Legacy runtime config preserved for compatibility while migrating."""
