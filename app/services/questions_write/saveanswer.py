...
def save_answer(params, *, session: Session):
    # try:
        ...
        if userid and orderid and message:
            ...
            check_order = session.query(OrderMess).filter_by(id=int(orderid)).first()

            check_author_userrole = session.query(UserBaseRole).filter_by(userid=int(userid)).first()

            author_user_role = get_role(check_author_userrole.roleid)

            check_answer = session.query(AnswerMess).filter(AnswerMess.orderid == int(orderid)).first()

            check_inwork = session.query(OrdersInWork).filter(OrdersInWork.orderid == int(orderid)).first()

            order_status_rec = session.query(OrderStatus).filter(OrderStatus.orderid == int(orderid)).first()

            order_space_rec = session.query(OrderSpace).filter_by(orderid=int(orderid)).first()
            ...
            answer_user_telinfo = session.query(UserTelegramInfo).filter_by(userid=int(userid)).first()
            ...
            if len(uploaded_files) == 0:
                ...
                for file_item in uploaded_files:
                    ...
                    if file_size < int(app_conf_rec.uploadsize):
                        ...
                        session.add(new_answer_attach)
                        session.commit()

            attachments = []

            for at_item in session.query(AnswerAttachment).filter(AnswerAttachment.answerid == answer_id).all():
            ...
            check_anonym_quest = session.query(AnonymOrder).filter_by(orderid=int(orderid)).first()

            if check_anonym_quest is not None:

                anon_quest_info = session.query(AnonymOrderInfo).filter_by(orderid=int(orderid)).first()
                ...
                check_tel_info = session.query(UserTelegramInfo).filter_by(userid=check_order.userid).first()

                check_status = session.query(OrderStatus).filter_by(orderid=check_order.id).first()
