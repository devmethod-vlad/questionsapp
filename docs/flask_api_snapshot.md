...
## 7) Черновик тикетов для следующего шага 1.2

1. Написать contract-tests для всех endpoint в разделе 2.
2. Добавить snapshot тесты для action-веток `/service`, `/statistic`, `/botexcel`.
3. Зафиксировать реальные JSON-схемы ответов сервисов через интеграционные тесты.
4. Отдельно покрыть multipart endpoint `/saveorupdate/` (с/без файлов).
5. Добавить негативные сценарии по неверным типам/отсутствующим полям.
