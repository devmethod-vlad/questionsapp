...
def get_perdayphrazes_stat(delta, downloadflag):
    try:
        ...
        dfGroup = df.groupby(['created_at']).agg({'searchphraze': 'count'})
        ...
        if int(downloadflag) == 1:
            dfGroup = dfGroup.rename(columns={"created_at": "ДАТА", "searchphraze": "КОЛ-ВО ЗАПРОСОВ"})
            wb = Workbook()
            ws = wb.active
            ...
            return send_file(out, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', as_attachment=True,
                             download_name="perdayphrazestat.xlsx")
        else:
            dfGroup = dfGroup.rename(columns={"created_at": "x", "searchphraze": "y"})
            infodata = dfGroup.to_dict('records')
            return {'status': 'ok', 'info': {'phrazes_data': infodata}}
